...
\begin{center}
  ...
  \begin{tabular}{rl}
    ...
    \textbf{Прошивка:}   & EdgeTX <<Jolly Mon>> v2.11.5 (пульт) / ExpressLRS v4.0.0 (RF-модуль) \\
...
\begin{enumerate}[nosep]
  ...
  \item Выбрать целевое устройство: \textbf{RadioMaster GX12 2.4/900 TX} (категория RadioMaster Dual Band).
  \item Выбрать версию прошивки ExpressLRS (текущая --- \textbf{v4.0.0}) и нужные опции:
  \begin{itemize}[nosep]
    \item Регуляторные домены: \textbf{2.4 GHz ISM (Стандарт)} + \textbf{868 MHz Европа}
    \item Задать \textbf{Binding Phrase} (одинаковую для TX и RX)
  \end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item В Configurator выбрать целевое устройство: \textbf{RadioMaster DBR4-TD True Diversity Dual Band RX}.
  \item Собрать и прошить прошивку (Build \& Flash или через Wi-Fi).
\end{enumerate}

Метод прошивки --- \textbf{EdgeTX Passthrough}: пульт включён и подключён по USB-C, в Configurator выбран порт вида \texttt{/dev/tty.usbmodem...OpenTX}.

После обновления пульта и приёмника убедитесь, что версии ExpressLRS совпадают (обе v4.0.0). При использовании одинаковой \textbf{Binding Phrase} связывание происходит автоматически; в противном случае выполните связывание вручную (см. раздел~3.2).
...
\begin{longtable}{ll}
  ...
  Скорость пакетов & до 500 Hz (1000 Hz в ExpressLRS v3.4+ / v4.0) \\
  ...
  Прошивка         & ExpressLRS v3.3.2 (предустановлена), обновлена до v4.0.0 \\
  \bottomrule
\end{longtable}
